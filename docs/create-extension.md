A Showdown extension is a function that returns an array of sub-extensions (henceforth called "sub-extensions"). The modern, recommended kind is the **`listener`** sub-extension, which hooks the [event system](event-system.md); the older `lang` and `output` kinds are still supported but **deprecated**.
...
* for the legacy `lang`/`output` types: either [`regex` and `replace` properties](#regex-and-replace) or a [`filter` property](#filter).

### Type

**Type** is a **required** property that defines the nature of the corresponding sub-extensions. It takes one of three values, `listener` being the recommended one:

* **`listener`**: listener extension to hook the [event system](event-system.md) and inspect or modify a sub-parser's captures, matches, attributes, or output mid-conversion. This is the **main extension mode** and the one to reach for in new code.

    Unlike the legacy `lang`/`output` extensions, a `listener` extension does not use `regex`/`replace` or `filter`. Instead, it defines a [`listeners`](#listener-extensions) object. See [Listener extensions](#listener-extensions) below.
...
The remaining two types are the **legacy** extension modes. They still work, but each one now runs as a thin wrapper over a document-level [event](event-system.md#makehtml-document-level-events), and loading one logs a deprecation warning — prefer a `listener` extension instead.

* **`lang`** _(deprecated)_: language extension to add new Markdown syntax to Showdown.
    ...
    !!! warning "Deprecated"
        `lang` extensions are now sugar over the [event system](event-system.md): each one is registered as a listener on the [`makehtml.onPreParse`](event-system.md#makehtml-document-level-events) event. They are **deprecated** and loading one logs a deprecation warning — prefer writing a `listener` extension on `makehtml.onPreParse` (or `makehtml.onStart` if you need the raw, pre-escape source) directly.

* **`output`** _(deprecated)_: output extension (or modifier) to alter the HTML output generated by Showdown.
    ...
    !!! warning "Deprecated"
        `output` extensions are now sugar over the [event system](event-system.md): each one is registered as a listener on the [`makehtml.onEnd`](event-system.md#makehtml-document-level-events) event. They are **deprecated** and loading one logs a deprecation warning — prefer writing a `listener` extension on `makehtml.onEnd` directly.
...
!!! warning "Legacy modes"
    The `regex`/`replace` and `filter` properties described in this and the next section apply **only** to the deprecated [`lang` and `output`](#type) extension types. In new code, prefer a [`listener` extension](#listener-extensions) instead.
